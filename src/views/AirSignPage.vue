...
export default {
  ...
  setup () {
    ...
    const { signProcess, ifHold, drawSign } = useHashSignCapture(
      landmarks,
      detected
    )
    // 簽名狀態 定義以hash模式的簽名方式
    const writePhase = ref(0)
    // 界面狀態紀錄與送出訊息框
    const signState = ref('準備簽名')
    ...
    function checkHold () {
      ...
      if (holdSecond.value && Date.now() - holdSecond.value - 1500 > 1500) {
        if (writePhase.value === 0) {
          signState.value = '簽名開始'
          writePhase.value = 1
        } else if (writePhase.value === 1 && store.state.handLandmarkPosition.length > 50) {
        ...
        //     type: 'success'
        //   })
        //   store.dispatch('loginAndGetToken')
        // }
        // sentLock.value = false
      } else if (holdSecond.value && Date.now() - holdSecond.value > 1500) {
        if (writePhase.value === 0) {
          signState.value = '準備開始'
        } else if (writePhase.value === 1) {
          signState.value = '準備送出'
        }
