...
export default function useSignCapture (landmarks, detected) {
  ...
  function drawSign (ctx) {
    ...
    // 遍歷抓取筆畫
    // 應對沒有任何筆畫的問題
    if (store.state.handLandmarkPosition.length === 0) {
      return 0
    }
    // 新路徑
    ctx.beginPath()
    ...
    ctx.moveTo(
      store.state.handLandmarkPosition[0].landmark[8].x * ctx.canvas.width,
      store.state.handLandmarkPosition[0].landmark[8].y * ctx.canvas.height
    )
    // 遍歷這條線裡的所有點
    for (let i = 1; i < store.state.handLandmarkPosition.length; i++) {
      // 畫線到此點
      ctx.lineTo(
        store.state.handLandmarkPosition[i].landmark[8].x * ctx.canvas.width,
        store.state.handLandmarkPosition[i].landmark[8].y * ctx.canvas.height
      )
    }
    // 建立線條（沒有這個看不到線條）
    ctx.stroke()
  ...
  const ifHold = computed(() => {
    ...
    if (!detected.value) {
      ...
      for (let i = 0; i < CURR.length; i++) {
        if (Math.abs(CURR[i].x - PREV[i].x) > 0.007) {
          return '運動中'
        }
        if (Math.abs(CURR[i].y - PREV[i].y) > 0.007) {
          return '運動中'
        }
      }
    }
